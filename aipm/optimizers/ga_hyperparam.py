...
def fixTypes(param_types,parameters):
    for t, fun in param_types.items():
        parameters[t] = fun(parameters[t])


def _evalFunction(individual, searchobj, name_values, X, y, scorer, cv, iid, fit_params,
                  verbose=0, error_score='raise',param_types={}):
    parameters = _individual_to_params(individual, name_values)
    score = 0
    n_test = 0

    fixTypes(param_types,parameters)
...
class EvolutionaryAlgorithmSearchCV(BaseSearchCV):
    ...
    def fit(self, X, y=None):
        ...
        self._fit(X, y, self.possible_params)
        if self.refit:
            self.best_estimator_ = clone(self.estimator)
            fixTypes(self.gene_type, self.best_params_)
            self.best_estimator_.set_params(**self.best_params_)
            self.best_estimator_.fit(X, y)
            
            self.population_estimators=[]            
            for est in self.population_:
                dummy = clone(self.estimator)
                fixTypes(self.gene_type, est[0])
                dummy.set_params(**est[0])
                dummy.fit(X,y)
                self.population_estimators.append(dummy)
    ...
    def _fit(self, X, y, parameter_dict):
        ...
        toolbox.register("evaluate", _evalFunction, searchobj=self,
                         ...
                         error_score=self.error_score, fit_params=self.fit_params,param_types=self.gene_type)
        ...
        hof = tools.HallOfFame(self.population_size*self.generations_number)
        ...
        pop, logbook = algorithms.eaSimple(pop, toolbox, cxpb=self.gene_crossover_prob, mutpb=self.gene_mutation_prob,
        ...
        dummy=[]
        for i in range(0,len(hof)-1):
            dummy.append((_individual_to_params(hof[i], name_values),hof[i].fitness.values[0]))

        self.population_=dummy

...
class StackedGeneralizer(object):
    ...
    def __init__(self, optimizer,base_models=None, blending_model=None, n_folds=5, verbose=True,extra_data=True,task=None,optimizer_params={},method_params={}):
        """
        ...
        verbose: boolean
        optimizer: whether the model inherits from the class 'Optimizer'
        ...
        for i, model in enumerate(self.base_models):
            ...
            for j, (train_idx, test_idx) in enumerate(kf):
                ...
                try:
                    ...
                    #if pandas dataframe then call .values
    ...
    def transform_blending_model(self, X_blend,data=None):
        ...
        for i, model in enumerate(self.blending_model_cv):
            cv_predictions = None
            if self.extra_data:
                model_predictions = get_predictions(model, np.column_stack([X_blend,data]))                
            else:
    ...
    def predict(self, X):
        ...
        if self.extra_data:
            predictions = self.transform_blending_model(X_blend,data=X)            
        else:
    ...
    def _fit(self, X, y):
        ...
        if self.extra_data:
            self.fit_blending_model(np.column_stack([X_blend,X]), y)
        else:
    ...
    def fit_optimizer(self, X, y):
        ...
        if self.extra_data:
            self.blending_model=self.blending_model(train=np.column_stack([X_blend,X]),target=y,task=self.task,**self.optimizer_params)
            self.blending_model.optimizeModelGA(**self.method_params)
            self.fit_blending_model(np.column_stack([X_blend,X]), y)
        else:
            self.blending_model=self.blending_model(train=X_blend,target=y,task=self.task,**self.optimizer_params)
            self.blending_model.optimizeModelGA(**self.method_params)
            self.fit_blending_model(X_blend, y)
